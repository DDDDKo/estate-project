...
# - 이메일 인증번호(이메일, 인증번호)
# - 게시물 (접수 번호, 상태, 제목, 작성자, 작성일, 조회수, 내용, 답변)
# - 부동산 정보 (번호, 시/도, 시/군/구, 매매 평균, 보증금 평균, 월세 평균, 날짜, 수익률 평균 전체, 수익률 평균 전체 40 초과, 수익률 평균 전체 40 이하, 매매가, 매매가격 대비 전세 비율 전체, 매매가격 대비 전세 비율 40 초과, 매매가격 대비 전세 비율 40 이하, 전세가, 전세가격 대비 월세 보증금 비율 전체, 전세가격 대비 월세 보증금 비율 40 초과, 전세가격 대비 월세 보증금 비율 40 이하)

# Relationship
# 사용자 - 이메일 인증번호 : 이메일 인증번호 테이블에 등록된 사용자의 이메일로 사용할 수 있음 (1 : 1)
...
# - 게시물 (접수 번호, 상태, 제목, 내용, 작성자, 작성일, 조회수, 답변)
# table name : board
...
CREATE TABLE user (
    ...
    join_path VARCHAR(5) NOT NULL 
        DEFAULT('HOME') 
        CHECK(join_path  IN ('HOME', 'NAVER', 'KAKAO')),
